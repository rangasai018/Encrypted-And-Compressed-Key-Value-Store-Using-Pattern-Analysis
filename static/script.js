...
async function initializeApp() {
    ...
    if (storeForm) storeForm.addEventListener('submit', handleStore);
    if (retrieveForm) retrieveForm.addEventListener('submit', handleRetrieve);
...
function switchTab(tabName) {
    ...
    if (tabName === 'analytics') loadStats();
}

// Keys
async function loadKeys() {
    try {
        const response = await fetch('/keys', { headers: getAuthHeaders() });
        const result = await response.json();
        displayKeys(result.keys || []);
    } catch (error) {
...
function displayKeys(keys) {
    ...
    keysList.innerHTML = keys.map(key => `
        <div class="key-item" onclick="retrieveKey('${key}')">${key}</div>
    `).join('');
}
...
async function retrieveKey(key) {
    document.getElementById('retrieveKey').value = key;
    switchTab('operations');
    setTimeout(() => document.getElementById('retrieveForm').dispatchEvent(new Event('submit')), 100);
}

// Backend info
async function loadBackendInfo() {
    try {
        const response = await fetch('/stats', { headers: getAuthHeaders() });
        const result = await response.json();
        document.getElementById('backend-info').textContent = `Keys: ${result.total_keys || 0}`;
    } catch (error) {
        console.log("Backend info error:", error);
    }
}
...
async function handleStore(e) {
    ...
    const response = await fetch('/store', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ key, value, encrypt, compress })
    });
...
async function handleRetrieve(e) {
    ...
    const response = await fetch(`/retrieve/${encodeURIComponent(key)}`, {
        headers: getAuthHeaders()
    });
...
// Analysis
async function handleAnalyze() {
    const response = await fetch('/analysis', { headers: getAuthHeaders() });
    const result = await response.json();
    document.getElementById('analysisResult').innerHTML = JSON.stringify(result, null, 2);
}
